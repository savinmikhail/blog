...
> **Modulith** — архитектурный стиль, при котором приложение остаётся одним исполняемым модулем, но код внутри разбит на независимые модули. Это помогает избежать хаоса в больших проектах без необходимости перехода на microservices.
...
Классическая структура Symfony:
```shell
├── src
   ├── Command
   ├── Controller
...
Подход modulith позволяет сохранить монолитную природу приложения, не жертвуя масштабируемостью и поддерживаемостью. Модули изолируют бизнес-логику, упрощают тестирование и дают ощущение порядка в коде. Такой подход отлично подходит для командной разработки и подготовки к возможному переходу на микросервисы в будущем — если он вообще потребуется.
